...
pub fn trans_method_callee<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    match origin {
        ...
        typeck::MethodParam(typeck::MethodParam {
            ...
            let origin = match
                traits::fulfill_obligation_post_typeck(bcx.tcx(),
                                                       span,
                                                       (*trait_ref).clone())
            {
                Ok(o) => o,
                Err(e) => {
                    bcx.sess().span_bug(
                        span,
                        format!("fulfilling obligation {} failed: {}",
                                trait_ref.repr(bcx.tcx()),
                                e.repr(bcx.tcx())).as_slice());
                }
            };
                                  ...
                                  -> ValueRef
{
    ...
    let trait_ref = Rc::new(ty::TraitRef { def_id: trait_id,
                                           substs: trait_substs });
    let vtbl = traits::fulfill_obligation_post_typeck(bcx.tcx(),
                                                      DUMMY_SP,
                                                      trait_ref).unwrap();
        ...
        _ => {
            bcx.tcx().sess.bug(
                format!("static call to invalid vtable: {}",
                        vtbl.vtable.repr(bcx.tcx())).as_slice());
        }
    }
}
                  ...
                  -> ValueRef
{
    ...
    let methods = traits::supertraits(tcx, trait_ref.clone()).flat_map(|trait_ref| {
        let origin = {
            match traits::fulfill_obligation_post_typeck(bcx.tcx(),
                                                         DUMMY_SP,
                                                         trait_ref.clone()) {
                Ok(o) => o,
                Err(e) => {
                    bcx.sess().bug(
                        format!("error resolving {}: {}",
                                trait_ref.repr(bcx.tcx()),
                                e.repr(bcx.tcx())).as_slice());
                }
            }
